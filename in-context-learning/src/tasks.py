...
class NumBaseConversion(Task):
    def __init__(self, n_dims, batch_size, pool_dict=None, seeds=None, noise=False):
        """scale: a constant by which to scale the randomly sampled weights."""
        ...
        if pool_dict is None and seeds is None:
            self.bases = torch.randn(self.b_size, self.n_dims, 1)
        elif seeds is not None:
            self.bases = torch.zeros(self.b_size, self.n_dims, 1)
            generator = torch.Generator()
            assert len(seeds) == self.b_size
            for i, seed in enumerate(seeds):
                generator.manual_seed(seed)
                self.bases[i] = torch.randn(self.n_dims, 1, generator=generator)
        else:
            assert "bases" in pool_dict
            indices = torch.randperm(len(pool_dict["w"]))[:batch_size]
            self.bases = pool_dict["w"][indices]

    def evaluate(self, xs_b):
        ys_b = np.vectorize(lambda x: int(np.base_repr(x, 2)))(xs_b.numpy())
        ys_b = torch.from_numpy(ys_b).view_as(xs_b)[:,:,0]
        return ys_b.to(xs_b.device)

    @staticmethod
    def generate_pool_dict(n_dims, num_tasks, **kwargs):  # ignore extra args
        return {"bases": torch.randn(num_tasks, n_dims, 1)}
